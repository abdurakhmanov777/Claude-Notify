...
| `claudeNotify.actionButtons` | `true` | Кнопка «Выключить уведомления» в пуше — гасит уведомления как переключатель в статус-баре (через управляющий топик). При выключении приходит тихий пуш с кнопкой «Включить», так что вернуть можно **с телефона**. В основном Android. |
...
- **Claude Notify: молчание по активности (вкл/выкл)** — переключить, слать ли пуш о завершении, когда вы работаете в VS Code (то же, что `idleSeconds`)
- **Claude Notify: кнопка в уведомлении (показать/скрыть)** — показывать ли в пуше кнопку «Выключить уведомления» (то же, что `actionButtons`)
