...
| `claudeNotify.interruptedMessage` | `Запрос прерван` | Текст, если запрос прервался или упёрся в лимит токенов. Поддерживает `{project}`. |
...
- **Claude Notify: молчание при активности (вкл/выкл)** — включить/выключить пропуск пуша о завершении, когда вы работаете в VS Code. По умолчанию выключено; при включении команда каждый раз спрашивает, сколько секунд бездействия требовать (записывается в `idleSeconds`)
